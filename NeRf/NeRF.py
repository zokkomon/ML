...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 10 14:07:21 2024
...
class NeRFmodel(nn.Module):
    def __init__(self, dim_pos=10, dim_dir=4, hid_dim=128):
        super(NeRFmodel, self).__init__()
        
        self.block1 = nn.Sequential(nn.Linear(dim_pos * 6 + 3, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim), nn.ReLU(),
                                    )
        # density estimation
        self.block2 = nn.Sequential(nn.Linear(dim_pos * 6 + hid_dim + 3, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim), nn.ReLU(),
                                    nn.Linear(hid_dim, hid_dim + 1)
                                    )
        # color estimation
        self.block3 = nn.Sequential(nn.Linear(dim_dir * 6 + hid_dim + 3, hid_dim // 2), nn.ReLU(), )
        self.block4 = nn.Sequential(nn.Linear(hid_dim // 2, 3), nn.Tanh(), )
    ...
    def forward(self, o, d):
        emb_x = self.positional_encoding(o, self.dim_pos) # emb_x: [batch_size, embedding_dim_pos * 6]
        emb_d = self.positional_encoding(d, self.dim_dir) # emb_d: [batch_size, embedding_dim_direction * 6]
